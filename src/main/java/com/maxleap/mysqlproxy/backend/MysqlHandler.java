package com.maxleap.mysqlproxy.backend;
...
public class MysqlHandler implements Handler<Buffer> {
    ...
    public MysqlHandler(NetSocket socket){
...
//                    completionHandler.handle(CommandResponse.failure(new UnsupportedOperationException("Unsupported authentication method: " + authMethodName)));
    ...
    private void sendSplitPacket(ByteBuf packet) {
        ...
        while (payload.readableBytes() >= PACKET_PAYLOAD_LENGTH_LIMIT) {
            ...
            clientSocket.write(Buffer.buffer( packetHeader ));
            clientSocket.write( Buffer.buffer( payload.readRetainedSlice(PACKET_PAYLOAD_LENGTH_LIMIT)) );
        }
    ...
    void sendNonSplitPacket(ByteBuf packet) {
        sequenceId++;
        clientSocket.write(Buffer.buffer(packet));
    }
